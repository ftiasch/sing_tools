    ...
    def __init__(self, provider, config):
        ...
        match parsed_url.scheme:
            case "ss":
                ...
                # SIP002 also allows plain method:password (percent-encoded)
                netloc = parsed_url.netloc
                if "@" not in netloc:
                    raise ValueError("Invalid SS URL format")
                method_password, server_port = netloc.split("@", 1)
                # Try base64 first, fall back to percent-decoded plain form
                try:
                    decoded = b64decode(method_password)
                except Exception:
                    from urllib.parse import unquote as _unquote
                    decoded = _unquote(method_password)
                if ":" not in decoded:
                    raise ValueError("Invalid SS URL format")
            ...
            case "vless":
                ...
                if transport in ("xhttp",):
                    raise ValueError(f"Unsupported VLESS transport: {transport}")
                ...
                # Flow: copy from URL only if non-empty
                if flow:
                    self.sing["flow"] = flow
                ...
                if tls_enabled:
                    ...
                    if alpn_raw:
                        tls_config["alpn"] = alpn_raw.split(",")
                    if security == "reality":
                        pbk = qs.get("pbk", [""])[0]
                        sid = qs.get("sid", [""])[0]
                        tls_config["reality"] = {
                ...
                if transport == "grpc":
                    ...
                    host = qs.get("host", [""])[0]
                    path = qs.get("path", [""])[0]
                    transport_config = {"type": transport}
                    if path:
                        transport_config["path"] = path
                    if host and transport == "ws":
            ...
            case "hy2" | "hysteria2":
                ...
                hop_interval = qs.get("hopinterval", [""])[0]
                self.__name = unquote(parsed_url.fragment, encoding="utf-8")
                self.sing = {
                    **SING_DIAL,
                    "type": "hysteria2",
                ...
                if mport:
                    self.sing["server_ports"] = [mport.replace("-", ":")]
                if hop_interval:
                    self.sing["hop_interval"] = hop_interval
            case "vmess":
                # vmess://base64(JSON)
